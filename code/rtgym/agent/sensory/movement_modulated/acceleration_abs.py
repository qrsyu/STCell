...
class AccelerationAbs(MMBase):
    ...
    def get_responses(self, traj: rtgym.trial.Trajectory):
        disps = traj.disps
        acc = np.diff(disps, axis=1)
        acc = np.concatenate((acc, np.zeros((acc.shape[0], 1, acc.shape[-1]))), axis=1)
        dup_acc = self._duplicate_res(acc)
        return self._smooth_res(dup_acc)
